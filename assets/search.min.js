(function($) {
  ...
  var LunrSearch = (function() {
    ...
    LunrSearch.prototype.displayResults = function(entries, query) {
      ...
      $entries.empty();
      $entries.append('<div id="search-entries-user"></div><div id="search-entries-api"></div>');

      if (entries.length > 0) {
        var search_result = this.template({entries: entries});
        var search_query = query.replace(/(\s+)/,"(<[^>]+>)*$1(<[^>]+>)*");
        var pattern = new RegExp("("+search_query+")", "gi");
        search_result = search_result.replace(pattern, "<b>$1</b>");
        search_result = search_result.replace(/(<b>[^<>]*)((<[^>]+>)+)([^<>]*<\/b>)/,"$1</b>$2<b>$4");
        $entries.append(search_result);
      }
      $(".hf.page #search-entries-api").prepend($(".search-result-api"));
      $(".hf.page #search-entries-user").prepend($(".search-result-user"));
      if ($(".search-result-user").length === 0) {
	$(".hf.page #search-entries-user").prepend('<p>No results.</p>');
      }
      if ($(".search-result-api").length === 0) {
	$(".hf.page #search-entries-api").prepend('<p>No results.</p>');
      } 
      if (typeof isPageAPI == 'undefined') { 
	$(".hf.page #search-entries-api, .hf.page #search-entries-user").show();
      } else if (isPageAPI){ 
	$(".hf.page #search-entries-api").show();
	$(".hf.page #search-entries-user").hide();
      } else if (!isPageAPI)  {
	$(".hf.page #search-entries-api").hide();
	$(".hf.page #search-entries-user").show();
      }
      $results.show();
    };
